...
Four tests, each designed to kill a strategy that only looks good.
A strategy that passes all four still might not work. A strategy that fails
any of them almost certainly does not.
"""
...
def verdict(wf_sharpe: float, folds_positive: float, sens: pd.DataFrame,
            rand_pct: float, breakeven_bps: float, live_bps: float) -> list[str]:
    notes = []
